...
const OfferBox: React.FC = () => {
  ...
  const goToCheckout = (url: string, planName: string, value: number) => {
    ...
    try {
      ...
      // "Capturar automaticamente todos os parâmetros da URL atual... e qualquer outro parâmetro existente."
      currentUrlParams.forEach((val, key) => {
        // "Não duplicar parâmetros... Se já existir no checkout, manter o valor original."
        if (!checkoutUrlParams.has(key)) {
      ...
      // Fallback em caso de erro na manipulação da URL (navegadores muito antigos)
